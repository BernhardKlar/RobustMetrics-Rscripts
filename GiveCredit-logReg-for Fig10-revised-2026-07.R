####################################################
## Data example in Section 7, Logistic Regression ##
####################################################
...
time.fit.sub = system.time({
  LR.train1 = glm(SeriousDlqin2yrs ~ ., family=binomial, data=training1[,-c(6,11)])
})["elapsed"]

###############################################################################
# computation of empirical optimal threshold for regression function
time.pred.train.sub = system.time({
  cg1 = predict(LR.train1, type = 'response')
})["elapsed"]
...
time.pred.test.full = system.time({
  cg1 = predict(LR.train, newdata=test[,-c(6,11)], type = 'response')
})["elapsed"]
time.pred.test.full
...
del.vec = seq(0.001,0.999,0.001); nd = length(del.vec)
M.res = Fgen.res = MCCgen.res = matrix(0, nrow=nd, ncol=6)
pi.test.lrf = matrix(0, nrow=nd, ncol=2)
n = length(test[,1])
...
for (i in 1:nd) {
  ...
  cf1$table
  pi1 = c(cf1$table[1,1]/n1, cf1$table[2,2]/n0)
  pi.test.lrf[i,] = pi1 #save result for ROC curve 
  #
  M.res[i, 1] = JAC.metric(pi1[1], pi1[2], pr)
  M.res[i, 2] = MCC.metric(pi1[1], pi1[2], pr)
  M.res[i, 3] = Yule.metric(pi1[1], pi1[2], pr)
...
#
tab1.lrf = tab1; tab2.lrf = tab2; tab3.lrf = tab3 #save results for ROC curve
#
...
time.pred.test.sub = system.time({
  cg1 = predict(LR.train1, newdata=test1[,-c(6,11)], type = 'response')
})["elapsed"]
...
del.vec = seq(0.001,0.999,0.001); nd = length(del.vec)
M.res = Fgen.res = MCCgen.res = matrix(0, nrow=nd, ncol=6)
pi.test.lrs = matrix(0, nrow=nd, ncol=2)
n = length(test1[,1])
...
for (i in 1:nd) {
  ...
  cf1$table
  pi1 = c(cf1$table[1,1]/n1, cf1$table[2,2]/n0)
  pi.test.lrs[i,] = pi1 #save results for ROC curve  #
  M.res[i, 1] = JAC.metric(pi1[1], pi1[2], pr)
  M.res[i, 2] = MCC.metric(pi1[1], pi1[2], pr)
  M.res[i, 3] = Yule.metric(pi1[1], pi1[2], pr)
  M.res[i, 4] = F.metric(pi1[1], pi1[2], pr, beta=1.5)
...
#
tab1.lrs = tab1; tab2.lrs = tab2; tab3.lrs = tab3 #save results for ROC curve
#
