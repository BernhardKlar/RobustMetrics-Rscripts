########################################
## Data example in Section 7, xgboost ##
########################################
...
################################################################################
## Load packages
library(xgboost); library(caret); library(sampling); library(xtable)
...
time.fit.full = system.time({
  dtrain = xgb.DMatrix(data = as.matrix(training[,-c(1,6,11)]), label = training$SeriousDlqin2yrs, missing = NA)
  XG.train = xgb.train(params = list(objective = "binary:logistic",
...
time.pred.train.full = system.time({
  cg.train = predict(XG.train, dtrain)
})["elapsed"]
...
time.fit.sub = system.time({
  dtrain1 = xgb.DMatrix(data = as.matrix(training1[,-c(1,6,11)]), label = training1$SeriousDlqin2yrs, missing = NA)
  XG.train1 = xgb.train(params = list(objective = "binary:logistic",
                                      max_depth = 4, learning_rate = 0.05,
                                      subsample = 0.8, colsample_bytree = 0.8,
                                      nthread = 1),
...
time.pred.train.sub = system.time({
  cg.train1 = predict(XG.train1, dtrain1)
})["elapsed"]
...
####################################################################################
# Prediction on the untouched test set using the model fitted to the training set
time.pred.test.full = system.time({
  dtest = xgb.DMatrix(data = as.matrix(test[,-c(1,6,11)]), label = test$SeriousDlqin2yrs, missing = NA)
  cg1 = predict(XG.train, dtest)
})["elapsed"]
...
del.vec = seq(0.001,0.999,0.001); nd = length(del.vec)
M.res = Fgen.res = MCCgen.res = matrix(0, nrow=nd, ncol=6)
pi.test.gbf = matrix(0, nrow=nd, ncol=2)
n = length(test[,1])
...
for (i in 1:nd) {
  ...
  cf1$table
  pi1 = c(cf1$table[1,1]/n1, cf1$table[2,2]/n0)
  pi.test.gbf[i,] = pi1 #save result for ROC curve 
  #
  M.res[i, 1] = JAC.metric(pi1[1], pi1[2], pr)
  M.res[i, 2] = MCC.metric(pi1[1], pi1[2], pr)
  M.res[i, 3] = Yule.metric(pi1[1], pi1[2], pr)
...
#
tab1.gbf = tab1; tab2.gbf = tab2; tab3.gbf = tab3 #save results for ROC curve
#
...
time.pred.test.sub = system.time({
  dtest1 = xgb.DMatrix(data = as.matrix(test1[,-c(1,6,11)]), label = test1$SeriousDlqin2yrs, missing = NA)
  cg1 = predict(XG.train1, dtest1)
})["elapsed"]
...
del.vec = seq(0.001,0.999,0.001); nd = length(del.vec)
M.res = Fgen.res = MCCgen.res = matrix(0, nrow=nd, ncol=6)
pi.test.gbs = matrix(0, nrow=nd, ncol=2)
n = length(test1[,1])
...
for (i in 1:nd) {
  ...
  cf1$table
  pi1 = c(cf1$table[1,1]/n1, cf1$table[2,2]/n0)
  pi.test.gbs[i,] = pi1 #save result for ROC curve 
  #
  M.res[i, 1] = JAC.metric(pi1[1], pi1[2], pr)
  M.res[i, 2] = MCC.metric(pi1[1], pi1[2], pr)
  M.res[i, 3] = Yule.metric(pi1[1], pi1[2], pr)
...
#
tab1.gbs = tab1; tab2.gbs = tab2; tab3.gbs = tab3 #save results for ROC curve
#
