...
XG.train1 = xgb.train(params = list(objective = "binary:logistic",
                                    max_depth = 4, learning_rate = 0.05,
                                    subsample = 0.8, colsample_bytree = 0.8),
                     data = dtrain1, nrounds = 300, verbose = 1)
...
####################################################################################
# Fitting xgboost to the test set
set.seed(100)
dtest = xgb.DMatrix(data = as.matrix(test[,-c(1,6,11)]), label = test$SeriousDlqin2yrs, missing = NA)
XG.test = xgb.train(params = list(objective = "binary:logistic",
                                  max_depth = 4, learning_rate = 0.05,
                                  subsample = 0.8, colsample_bytree = 0.8),
                   data = dtest, nrounds = 300, verbose = 1)

###############################################################################
# computation of empirical optimal threshold for regression function
cg1 = predict(XG.test, dtest)
...
XG.test1 = xgb.train(params = list(objective = "binary:logistic",
                                   max_depth = 4, learning_rate = 0.05,
                                   subsample = 0.8, colsample_bytree = 0.8),
                    data = dtest1, nrounds = 300, verbose = 1)

###############################################################################
# computation of empirical optimal threshold for regression function
cg1 = predict(XG.test1, dtest1)
